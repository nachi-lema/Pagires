              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header">
                    <h5 style="width: 100%;padding: 20px"><b><?php extrait_compte_autre_header(); ?></b></h5>
                  </div>
                  <div class="card-body">
                      ...
                      <table class="table table-striped table-hover" id='table-r' style="width: 100%">
                        ...
                        <tbody style="border: #000 2px solid;">
                          <?php $resultats = $bd->query(
                            'SELECT id,ref_doc,compte_debit,compte_credit,libelle,debits_usd,credits_usd,debits_cdf,credits_cdf,date_doc FROM comptes_autres WHERE compte_credit = "'.$compte.'" AND societe="'. $id_societed.'"
                                UNION
                            SELECT id,ref_doc,compte_debit,compte_credit,libelle,debits_usd,credits_usd,debits_cdf,credits_cdf,date_doc FROM compte_od_tb WHERE compte_credit = "'.$compte.'" societe="'. $id_societed.'
                            '
                          ) ?>
                          ...
                          <tr colspan="5">
                            ...
                            <!--somme credit usd--->
                            <?php $tot_credit_usd_autre = $bd->query('SELECT SUM(credits_usd) FROM comptes_autres WHERE compte_credit = "' . $compte . '" ') ?>
                            <?php $tot_credit_usd_CptOD = $bd->query('SELECT SUM(credits_usd) FROM compte_od_tb WHERE compte_credit = "' . $compte . '" ') ?>

                            <?php $row_credit_usd_autre = $tot_credit_usd_autre->fetch_array() ?>
                            <?php $row_credit_usd_CptOD = $tot_credit_usd_CptOD->fetch_array() ?>
                            <td style="text-align: right;"><strong><?= number_format($row_glob2 = $row_credit_usd_autre[0] + $row_credit_usd_CptOD[0], 2) ?></strong></td>
                            <!--fin somme debits usd--->

                            <!--somme debits usd--->
                            <?php $tot_debit_usd_autre = $bd->query('SELECT SUM(debits_usd) FROM comptes_autres WHERE compte_credit = "' . $compte . '" ') ?>
                            <?php $row_debit_usd_autre = $tot_debit_usd_autre->fetch_array() ?>


                            <td class="" style="text-align: right;"><strong><?= number_format($row_glob1 = $row_debit_usd_autre[0], 2) ?></strong></td>
                            <!--fin somme debits usd--->

                            <!--somme credit cdf--->
                            <?php $tot_credit_cdf_autre = $bd->query('SELECT SUM(credits_cdf) FROM comptes_autres WHERE compte_credit = "' . $compte . '" ') ?>
                            <?php $tot_credit_cdf_CptOD = $bd->query('SELECT SUM(credits_cdf) FROM compte_od_tb WHERE compte_credit = "' . $compte . '" ') ?>
                            <?php $row_credit_cdf_autre = $tot_credit_cdf_autre->fetch_array() ?>
                            <?php $row_credit_cdf_CpteOD = $tot_credit_cdf_CptOD->fetch_array() ?>


                            <td class="" style="text-align: right;"><strong><?= number_format($row_glob4 = $row_credit_cdf_autre[0] + $row_credit_cdf_CpteOD[0], 2) ?></strong></td>
                            <!--somme credit cdf--->

                            <!--somme debits cdf--->
                            <?php $tot_debit_cdf_autre = $bd->query('SELECT SUM(debits_cdf) FROM comptes_autres WHERE compte_credit = "' . $compte . '" ') ?>
                            <?php $row_debit_cdf_autre = $tot_debit_cdf_autre->fetch_array() ?>

                            <td class="" style="text-align: right;"><strong><?= number_format($row_glob3 = $row_debit_cdf_autre[0], 2) ?></strong></td>
                          ...
                          <?php
                          $req6 = "SELECT * FROM compta_autres WHERE compte ='$compte' AND statut ='Actif'";
                          $res6 = $bd->query($req6);
