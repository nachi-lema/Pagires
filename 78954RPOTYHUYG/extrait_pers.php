              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header">
                    <h5 style="width: 100%;padding: 20px;font-size: 15px"><b>EXTRAIT-COMPTE_Personnel | <?= $compte ?> | <?= $personnel ?></b></h5>
                  </div>
                  <div class="card-body">
                    <div class="table-responsive">
                      <table class="table table-bordered table-hover" id='table-r' style="width: 100%">
                        <thead>
                        ...
                        <tbody style="border: #000 2px solid;">
                          <?php $resultats = $bd->query(
                            'SELECT id,ref_doc,compte_debit,compte_credit,libelle,debits_usd,credits_usd,debits_cdf,credits_cdf,date_doc FROM comptes_pers WHERE compte_credit ="'.$compte.'" AND nom_complet="'.$personnel.'"
                                UNION
                            SELECT id,ref_doc,compte_debit,compte_credit,libelle,debits_usd,credits_usd,debits_cdf,credits_cdf,date_doc FROM compte_od_tb WHERE compte_credit = "'.$compte.'" AND nom_credit="'.$personnel.'"
                            '
                          ) ?>
                          ...
                          <tr colspan="5">
                            ...
                            <!--somme credit usd--->
                            <?php $tot_credit_usd_personnes = $bd->query('SELECT SUM(credits_usd) FROM comptes_pers WHERE compte_credit ="'.$compte.'" AND nom_complet="'.$personnel.'"') ?>
                            <?php $tot_credit_usd_CptOD = $bd->query('SELECT SUM(credits_usd) FROM compte_od_tb WHERE compte_credit = "'.$compte.'" AND nom_credit="'.$personnel.'" ') ?>
                            ...
                            <td style="text-align: right;"><strong><?= number_format($row_glob2 = $row_credit_usd_personne[0] + $row_credit_usd_CptOD[0], 2) ?></strong></td>
                            <!--fin somme debits usd--->

                            <!--somme debits usd--->
                            <?php $tot_debit_usd_personnes = $bd->query('SELECT SUM(debits_usd) FROM comptes_pers WHERE compte_credit ="'.$compte.'" AND nom_complet="'.$personnel.'" ') ?>
                            <?php $tot_debit_usd_personnel = $tot_debit_usd_personnes->fetch_array() ?>


                            <td class="" style="text-align: right;"><strong><?= number_format($row_glob1 = $tot_debit_usd_personnel[0], 2) ?></strong></td>
                            <!--fin somme debits usd--->

                            <!--somme credit cdf--->
                            <?php $tot_credit_cdf_personnes = $bd->query('SELECT SUM(credits_cdf) FROM comptes_pers WHERE compte_credit ="'.$compte.'" AND nom_complet="'.$personnel.'" ') ?>
                            <?php $tot_credit_cdf_CptOD = $bd->query('SELECT SUM(credits_cdf) FROM compte_od_tb WHERE compte_credit = "'.$compte.'" AND nom_credit="'.$personnel.'" ') ?>
                            ...
                            <td class="" style="text-align: right;"><strong><?= number_format($row_glob4 = $row_credit_cdf_personne[0] + $row_credit_cdf_CpteOD[0], 2) ?></strong></td>
                            <!--somme credit cdf--->

                            <!--somme debits cdf--->
                            <?php $tot_debit_cdf_personnes = $bd->query('SELECT SUM(debits_cdf) FROM comptes_pers WHERE compte_credit = "'.$compte.'" AND nom_complet="'.$personnel.'" ') ?>
                            <?php $row_debit_cdf_personne = $tot_debit_cdf_personnes->fetch_array() ?>

                            <td class="" style="text-align: right;"><strong><?= number_format($row_glob3 = $row_debit_cdf_personne[0], 2) ?></strong></td>
                          ...
                          <?php
                          $req6 = "SELECT * FROM compta_pers WHERE compte ='$compte' AND nom_complet='$personnel' AND statut ='Actif'";
                          $res6 = $bd->query($req6);
                          ...
                          if ($res6->num_rows > 0) {
                            while ($row6 = $res6->fetch_assoc()) {
                              echo '<tr>' .
                                ...
                                '<td colspan="2" style="text-align:right">' . '<strong>' . number_format($row_glob5 = $row6['solde1_usd'], 2) . '&nbsp;' . 'USD' . '</strong>' . '</td>' .
                      ...
                      <div class="row w-100">
                        <fieldset class="col-lg-4 col-md-4 col-sm-12">
                          <p class="lien"><a href="liste_compte_pers.php" class="btn btn-primary" style="background-color: #44597b">Revenir à la page précédente</a></p>
                        </fieldset>
                        <fieldset class="col-lg-4 col-md-4 col-sm-12">
